...
namespace TubeStar
{
    public class Video : UniqueObject
    {
        ...
        public void FetchRandomImage(bool useCategory = true)
        {
            ...
            YouTubeAPI.GetRandomImagesAsync(searchString, randomImageCount, (videoIds) =>
            {
                if (videoIds != null && videoIds.Count > 0)
                {
                    var randomInt = RandomHelpers.RandomInt(videoIds.Count);
                    SetImageFromId(videoIds[randomInt]);
                }
                else
                {
                    //Limit to category
                    YouTubeAPI.GetRandomImagesAsync(Category.GetString(), randomImageCount, (fallbackVideoIds) =>
                    {
                        if (fallbackVideoIds != null && fallbackVideoIds.Count > 0)
                        {
                            var randomInt = RandomHelpers.RandomInt(fallbackVideoIds.Count);
                            SetImageFromId(fallbackVideoIds[randomInt]);
                        }
                        else
                        {
                            SetCategoryFallbackImage();
                        }
                    });
                }
            });
        ...
        public static string GetLocalCategoryFallbackImage(VideoCategory category)
        {
            switch (category)
            {
        ...
        public void SetCategoryFallbackImage()
        {
            try
            {
                string packUri = GetLocalCategoryFallbackImage(Category);
